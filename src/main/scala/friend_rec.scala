...
object friend_rec {
  ...
  val (nodes, connections) = (sc.parallelize(Seq(0, 1, 2, 3)),
    sc.parallelize(Seq(0 -> 1, 1 -> 0, 1 -> 2, 2 -> 3, 3 -> 0, 3 -> 1, 0 -> 3, 1 -> 3, 3 -> 2)))
  ...
  val mutual_friends = friends_lists.map {
    ...
    .groupByKey()
    .filter(x => x._2.forall(xx => xx != 0))
    .map(x => (x._1.toSeq, x._2.sum))
    .flatMap(x =>
      Seq((x._1.head, (x._1(1), x._2)), (x._1(1), (x._1.head, x._2)))
    )
    .groupByKey()
